...
const uint64_t FILE_TYPE_UNKONW		= 0;
...
public:
	...
	static ftype_t check(FILE* file);
	static ftype_t check(FILE* file, byte_t* data);
...
std::vector<filetype::type> filetype::typelist = std::vector<filetype::type>();
...
ftype_t filetype::check(FILE* file)
{
	byte_t buf[1024] = {0};
...
{
	//byte_t buf[] = {0xFF, 0xD8, 0xFF, 0xE0 };	//6-9 == JFIF
	//byte_t buf[] = {0xFF, 0xD8, 0xFF, 0xE1 };	//6-9 == Exif
	if(size < 4)return false;
...
int filetype::init()
{
	reg_type(FILE_TYPE_UNKNOWN, "unknown", "unknown file format", null);
...
ftype_t filetype_fromExt(const cstring<char>& filename)
{
	if(filename.empty()){
...
//通过数据格式判断文件类型
uint64_t filetype_fromData(const byte_t* buf, size_t size)
{
